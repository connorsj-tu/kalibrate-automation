...
import com.common.page_objects.Kalibrate_210_LocatorLibrary;
...
public class KMobileWeb_Hooks extends Hooks {

	private static boolean beforeAllHookExecuted = false;
	private static boolean dbExtendedEventsRequired = false;


	public KMobileWeb_Hooks() {
		super();
		
		System.out.println("In KMobileWeb_Hooks constructor");
	...
	public void beforeScenarioHook(Scenario scenario) {
		...
		if(Boolean.parseBoolean(Utils.getProperty("PERFORM_HTTP_CHECKS"))) {
        	...
        	reporterHelper.log("In beforeScenarioHook method, performing HTTP Checks as PERFORM_HTTP_CHECKS set to true...");
...
//			break;
    	case "2.10":
    		kMobileWebLocatorLibrary = new KMobileWeb_Generic_LocatorLibrary();
			break;
    	default :
    ...
    public void beforeAll() {
        ...
        if(!beforeAllHookExecuted) {
            ...
            if(Boolean.parseBoolean(Utils.getProperty("PERFORM_HTTP_CHECKS"))) {
            	...
            	reporterHelper.log("In beforeAll method, performing HTTP Checks as PERFORM_HTTP_CHECKS set to true...");
        		performHttpRequestCheck("DEFAULT_ADMINISTRATOR");
            }
            else
            	reporterHelper.log("Skipping http checks as PERFORM_HTTP_CHECKS is set to false in config");
...
//    		reporterHelper.processTimingsFile(true);
	...
	public void afterScenarioCloseKalibrate() {
		...
		if(!driver.toString().contains("null"))
		{
			...
			if(BrowserHelper.isElementPresent(driver, By.xpath("//body[@data-ng-app='Kalibrate']"))) {
				
				reporterHelper.log("\tConfirmed Kalibrate is open...");
				
				if(BrowserHelper.isElementPresent(driver, kalibrateLocatorLibrary.kalibrate_login_UsernameTextBox)) {
					reporterHelper.log("\tKalibrate login page indetified, no need to logout");
				}
				else if(BrowserHelper.isElementPresent(driver, kalibrateLocatorLibrary.kalibrate_main_WorkspaceMenuHeader)) {
					
					WebElement workspaceMenuElement = BrowserHelper.syncOnElement(driver, kalibrateLocatorLibrary.kalibrate_main_WorkspaceMenuHeader, "present");
					...
					// Logout of Kalibrate
			        BrowserHelper.syncOnElement(driver, kalibrateLocatorLibrary.kalibrate_main_HamburgerIcon, "present").click();
			        BrowserHelper.customSleep(1 * 1000);
			        BrowserHelper.syncOnElement(driver, kalibrateLocatorLibrary.kalibrate_main_MainMenuLogoutItem, "present").click();
